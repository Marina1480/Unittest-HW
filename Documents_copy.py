...
def get_doc_owner_name():
    user_doc_number = "2207 876234" # заменила вместо инпут
    print()
...
def add_new_shelf(shelf_number=''):
    if not shelf_number:
        shelf_number = '2' # поменяла вместо input
...
def delete_doc():
    user_doc_number = "2207 876234" # поменяла вместо инпут
    doc_exist = check_document_existance(user_doc_number)
    if doc_exist:
        for current_document in documents:
            doc_number = current_document['number']
...
def get_doc_shelf():
    user_doc_number = "11-2" # вместо инпут
    doc_exist = check_document_existance(user_doc_number)
    if doc_exist:
        for directory_number, directory_docs_list in directories.items():
...
def add_new_doc():
    new_doc_number = "2210 876234" # вместо input
    new_doc_type = "passport"  # вместо input
    new_doc_owner_name = "Владлена Пугачева" # вместо input
    new_doc_shelf_number = '3' # вместо input
    new_doc = {
        "type": new_doc_type,
